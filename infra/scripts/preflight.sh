...
#
# TRAILING COMMENTS ARE PART OF THE FORMAT (T-6.11, #335). The first version
# anchored the closing quote to end-of-line, so
#
#   access_group_id = "90b0..." # xenopsbase-admins
#
# read as absent -- and the caller reported a value that was sitting right
# there, which is the same class of wrong as the Hetzner image grep this script
# already carries a comment about. secrets.tfvars.example has annotated every
# value this way since it was written, so the format was always this; only the
# reader disagreed. Capturing [^"]* and discarding the rest of the line also
# stops the old greedy \(.*\) from running past the closing quote.
tfvar() {
  local file="$1" key="$2"
  [ -f "$file" ] || return 0
  ...
  edge)
    ...
    if [ -z "$ZONE" ] || [ -z "$ACCOUNT" ]; then
      ...
      if [ "$(tfbool "$VARS" manage_access)" = "true" ]; then
        ...
        # Still no probe for access/organizations. Terraform does not manage
        # the organisation, so failing a preflight over a permission the apply
        # never uses is the cry-wolf case cf_probe_auth exists to avoid.
        #
        # The GROUP is a different matter now, and this reasoning used to cover
        # it. It does not any more: since T-6.11 the team policy admits an
        # Access group by id rather than a list of addresses, so the module
        # depends on that group existing. A permission the apply never uses is
        # noise; a permission that reads the object the policy points at is not.

        # THE GROUP HAS TO EXIST (T-6.11, #335)
        #
        # This is the one failure the group arrangement can produce that the
        # list of addresses could not, and it is silent everywhere else.
        # Membership moved into Cloudflare so CI would stop planning an empty
        # include, and the trade was that Terraform no longer describes who is
        # admitted. So a group deleted or renamed away in the console is
        # invisible to a plan: the id is still a well-formed UUID, the policy
        # still applies, and the result is a policy that admits nobody -- this
        # card's own failure arriving by a different road.
        #
        # A GET, not a cf_probe_edit DELETE. The rule elsewhere in this script
        # is that a permission the module WRITES with needs a write probe; this
        # one is deliberately read-only, because Terraform must never manage
        # this group. Managing it would need the member addresses, and CI would
        # then plan to empty it -- the same defect one resource sideways.
        #
        #   200  the group is there                  -> pass
        #   404  the id resolves to nothing          -> fail; re-applying cannot fix it
        #   403  Access: Groups Read was removed     -> fail, and name it
        #   else not understood                      -> unproven, never a pass
        #
        # The 403 arm is not hypothetical: this probe was written, found the
        # token refused on exactly this endpoint, and was held back until the
        # permission was added. It stays so that losing it again is caught here
        # rather than by a policy that quietly admits nobody.
        GROUP="$(tfvar "$VARS" access_group_id)"
        if [ -z "$GROUP" ]; then
          bad "manage_access = true but access_group_id is not set"               "set it in env/$ENVIRONMENT.tfvars; terraform refuses this too, this is only the earlier message"
        else
          group_code="$(curl -s -o /dev/null -w '%{http_code}'             -H "Authorization: Bearer ${TF_VAR_cloudflare_api_token}"             "https://api.cloudflare.com/client/v4/accounts/$ACCOUNT/access/groups/$GROUP" || echo 000)"
          case "$group_code" in
            200)
              pass "Access group $GROUP exists"
              ;;
            404)
              bad "Access group $GROUP does not exist"                   "the team policy would admit nobody; check the Group ID under Zero Trust / Access controls / Policies, Rule groups tab"
              ;;
            401 | 403)
              bad "cannot read Access group $GROUP"                   "add to the token: Account / Access: Groups / Read -- without it, a deleted group is undetectable from here"
              ;;
            000)
              unknown "Access group $GROUP -- could not reach the API"                       "network or DNS problem, not a permission one"
              ;;
            *)
              unknown "Access group $GROUP -- unexpected HTTP $group_code"                       "existence NOT proven; treat this as unchecked"
              ;;
          esac
        fi
      fi
