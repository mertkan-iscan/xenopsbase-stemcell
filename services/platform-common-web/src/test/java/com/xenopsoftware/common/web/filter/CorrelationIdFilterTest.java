...
class CorrelationIdFilterTest {
    ...
    private String idFor(String inboundHeader) throws Exception {
        ...
        when(request.getHeader(CorrelationId.HEADER)).thenReturn(inboundHeader);

        filter.doFilterInternal(request, response, chain);

        ArgumentCaptor<String> captor = ArgumentCaptor.forClass(String.class);
        verify(response).setHeader(org.mockito.ArgumentMatchers.eq(CorrelationId.HEADER), captor.capture());
    ...
    void theIdIsRemovedFromMdcEvenWhenTheChainThrows() throws Exception {
        ...
        HttpServletResponse response = mock(HttpServletResponse.class);
        FilterChain chain = mock(FilterChain.class);
        when(request.getHeader(CorrelationId.HEADER)).thenReturn("abc123");

        List<String> seenInsideChain = new ArrayList<>();
        org.mockito.Mockito.doAnswer(invocation -> {
            seenInsideChain.add(MDC.get(CorrelationId.MDC_KEY));
        ...
        assertThat(MDC.get(CorrelationId.MDC_KEY)).as("a pooled thread must not carry the id onward").isNull();
    }
}
