...
class ExceptionTranslatorUnitTest {
    ...
    /**
     * Gateway-only, and checked BEFORE the profile so the answer is the same in dev and prod.
     *
     * <p>NoResourceFoundException puts the request URI in its own reason, and under WebFlux that URI
     * is absolute. Left alone, every 404 this gateway returns carried the public hostname, to the
     * caller and into the logs and aggregators behind it. The class comment records that
     * `path` and `instance` already say which path was asked for, so the URI here was a third copy
     * and the only one with an origin attached.
     */
    @Test
    void aMissingStaticResourceDoesNotEchoTheAbsoluteUrl() {
        // The absolute URI is the first argument, and it is what ends up inside the exception's own
        // reason string, which is what used to reach the caller.
        URI absolute = URI.create("https://app-dev.xenopsoftware.com/secret");

        for (String profile : new String[] { "dev", "prod" }) {
            String detail = detailFor(translatorFor(profile), new NoResourceFoundException(absolute, "/secret"));

            assertThat(detail).isEqualTo("No static resource for this path.");
            assertThat(detail).doesNotContain("xenopsoftware.com").doesNotContain("https://");
        }
    }
    ...
    void productionHidesAnythingNamingOurOwnPackages() {
        ...
        assertThat(
            detailFor(translator, new IllegalStateException("failed at com.xenopsoftware.gateway.service.DocumentService.load"))
